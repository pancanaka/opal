...
public class GP2xCommands extends AbstractCommands implements Commands {
    ...
    public ResponseAPDU select(byte[] aid) throws CardException {
        ...
        selectComm[ISO7816.OFFSET_CLA.getVal()] = (byte) 0x00; // (CLA) command class
        selectComm[ISO7816.OFFSET_INS.getVal()] = (byte) 0xA4; // (INS) SELECT command
        selectComm[ISO7816.OFFSET_P1.getVal()] = (byte) 0x04; // (P1) SELECT by name
        selectComm[ISO7816.OFFSET_P2.getVal()] = (byte) 0x00; // (P2) first or only occurrence
        selectComm[ISO7816.OFFSET_LC.getVal()] = (byte) aid.length; // (LC) data length
        ...
        if (resp.getSW() != ISO7816.SW_NO_ERROR.getVal()) {
            this.resetParams();
    ...
    public ResponseAPDU initializeUpdate(byte keySetVersion, byte keyId, SCPMode desiredScp) throws CardException {
        ...
        initUpdCmd[ISO7816.OFFSET_CLA.getVal()] = (byte) 0x80;
        initUpdCmd[ISO7816.OFFSET_INS.getVal()] = (byte) 0x50;
        initUpdCmd[ISO7816.OFFSET_P1.getVal()] = keySetVersion;
        initUpdCmd[ISO7816.OFFSET_P2.getVal()] = keyId;
        initUpdCmd[ISO7816.OFFSET_LC.getVal()] = (byte) this.hostChallenge.length;
    ...
    public ResponseAPDU externalAuthenticate(SecLevel secLevel) throws CardException {
        ...
        extAuthCmd[ISO7816.OFFSET_CLA.getVal()] = (byte) 0x84;
        extAuthCmd[ISO7816.OFFSET_INS.getVal()] = (byte) 0x82;
        extAuthCmd[ISO7816.OFFSET_P1.getVal()] = this.secMode.getVal();
        extAuthCmd[ISO7816.OFFSET_P2.getVal()] = (byte) 0x00;
        extAuthCmd[ISO7816.OFFSET_LC.getVal()] = (byte) 0x10;
    ...
    /* (non-Javadoc)
     * @see fr.xlim.ssd.opal.library.commands.Commands#getStatus(fr.xlim.ssd.opal.library.FileType, fr.xlim.ssd.opal.library.GetStatusResponseMode, byte[])
     */
    ...
    public ResponseAPDU[] getStatus(FileType fileType, GetStatusResponseMode responseMode, byte[] searchQualifier) throws CardException {
        ...
        if (this.secMode == SecLevel.NO_SECURITY_LEVEL) {
            ...
            getStatusCmd[ISO7816.OFFSET_CLA.getVal()] = (byte) 0x80;
        } else {
            dataSize = (byte) (searchQualifier.length + 8); // searchQualifier + C-MAC
            getStatusCmd = new byte[headerSize + dataSize];
            getStatusCmd[ISO7816.OFFSET_CLA.getVal()] = (byte) 0x84;
        }

        getStatusCmd[ISO7816.OFFSET_INS.getVal()] = (byte) 0xF2;
        getStatusCmd[ISO7816.OFFSET_P1.getVal()] = fileType.getVal();
        getStatusCmd[ISO7816.OFFSET_P2.getVal()] = responseMode.getVal();
        getStatusCmd[ISO7816.OFFSET_LC.getVal()] = dataSize;
        ...
        while (resp.getSW() == ISO7816.SW_MORE_DATA_AVAILABLE.getVal()) {
            UncipheredgetStatusCmd[ISO7816.OFFSET_P2.getVal()] = (byte) (responseMode.getVal() + (byte) 0x01); // Get next occurrence(s)
            if (this.secMode != SecLevel.NO_SECURITY_LEVEL) {
                byte[] data_cmac = new byte[headerSize + dataSize - 8]; // data used to generate C-MAC
    ...
    public ResponseAPDU deleteOnCardObj(byte[] aid, boolean cascade) throws CardException {
        ...
        deleteComm[ISO7816.OFFSET_CLA.getVal()] = (byte) ((this.getSecMode() == SecLevel.NO_SECURITY_LEVEL) ? 0x80 : 0x84); // (CLA) command class (GlobalPlatform command + secure messaging with GlobalPlatform format)
        deleteComm[ISO7816.OFFSET_INS.getVal()] = (byte) 0xE4; // (INS) DELETE command
        deleteComm[ISO7816.OFFSET_P1.getVal()] = (byte) 0x00; // (P1) the only and the last DELETE command
        deleteComm[ISO7816.OFFSET_P2.getVal()] = (cascade ? (byte) 0x80 : (byte) 0x00); // (P2) 0x00 : only delete the specified object
        //      0x80 : delete object and related objects
        deleteComm[ISO7816.OFFSET_LC.getVal()] = dataSize;   // (LC) data length

        deleteComm[ISO7816.OFFSET_CDATA.getVal()] = (byte) 0x4F; // the object being deleted is specified by its AID
        deleteComm[ISO7816.OFFSET_CDATA.getVal() + 1] = (byte) aid.length; // AID length
        System.arraycopy(aid, 0, deleteComm, ISO7816.OFFSET_CDATA.getVal() + 2, aid.length); // put the AID into deleteComm
    ...
    public ResponseAPDU installForLoad(byte[] packageAid, byte[] securityDomainAid, byte[] params) throws CardException {
        ...
        installForLoadComm[ISO7816.OFFSET_CLA.getVal()] = (byte) ((this.getSecMode() == SecLevel.NO_SECURITY_LEVEL) ? 0x80 : 0x84); // (CLA) command class (GlobalPlatform command + secure messaging with GlobalPlatform format)
        installForLoadComm[ISO7816.OFFSET_INS.getVal()] = (byte) 0xE6; // (INS) INSTALL command
        installForLoadComm[ISO7816.OFFSET_P1.getVal()] = (byte) 0x02; // (P1) For load
        installForLoadComm[ISO7816.OFFSET_P2.getVal()] = (byte) 0x00; // (P2) no information provided
        installForLoadComm[ISO7816.OFFSET_LC.getVal()] = dataSize; // (LC) data length

        installForLoadComm[ISO7816.OFFSET_CDATA.getVal()] = (byte) packageAid.length; // AID length
    ...
    public ResponseAPDU[] load(byte[] capFile, byte maxDataLength) throws CardException {
        ...
        for (int i = 0; i < nbBlock; i++) {
            ...
            cmd[ISO7816.OFFSET_CLA.getVal()] = (byte) ((this.getSecMode() == SecLevel.NO_SECURITY_LEVEL) ? 0x80 : 0x84);
            cmd[ISO7816.OFFSET_INS.getVal()] = (byte) 0xE8;
            cmd[ISO7816.OFFSET_P1.getVal()] = (byte) ((i == nbBlock - 1) ? 0x80 : 0x00);
            cmd[ISO7816.OFFSET_P2.getVal()] = (byte) i;
    ...
    public ResponseAPDU installForInstallAndMakeSelectable(byte[] loadFileAID,
        ...
        installForInstallComm[ISO7816.OFFSET_CLA.getVal()] = (byte) ((this.getSecMode() == SecLevel.NO_SECURITY_LEVEL) ? 0x80 : 0x84); // (CLA) command class (GlobalPlatform command + secure messaging with GlobalPlatform format)
        installForInstallComm[ISO7816.OFFSET_INS.getVal()] = (byte) 0xE6;    // (INS) INSTALL command
        installForInstallComm[ISO7816.OFFSET_P1.getVal()] = (byte) 0x0C;     // (P1) For install
        installForInstallComm[ISO7816.OFFSET_P2.getVal()] = (byte) 0x00;    // (P2) no information provided
        installForInstallComm[ISO7816.OFFSET_LC.getVal()] = dataSize;    // (LC) data length
