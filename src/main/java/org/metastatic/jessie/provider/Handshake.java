...
final class Handshake implements Constructed {
	...
	static Handshake read(InputStream in, CipherSuite suite, PublicKey key,
			CertificateType certType) throws IOException {
		...
		if (type == Type.HELLO_REQUEST) {
			...
			if (lenbuf[0] == 3 && (lenbuf[1] >= 0 && lenbuf[1] <= 2)) {
				...
				if (challenge.length > 32)
					...
					byte[] b = new byte[32];
					System.arraycopy(challenge, 0, b, b.length
							- challenge.length, challenge.length);
					challenge = b;
				}
				int time = (challenge[0] & 0xFF) << 24
						| (challenge[1] & 0xFF) << 16
				...
				return new Handshake(Handshake.Type.CLIENT_HELLO,
						new ClientHello(vers, rand, id, suites,
								Collections
										.singletonList(CompressionMethod.NULL)));
			}
	...
	static class Type implements Enumerated {
		...
		public static final Type HELLO_REQUEST = new Type(0),
				CLIENT_HELLO = new Type(1), SERVER_HELLO = new Type(2),
				CERTIFICATE = new Type(11), SERVER_KEY_EXCHANGE = new Type(12),
				CERTIFICATE_REQUEST = new Type(13),
				SERVER_HELLO_DONE = new Type(14),
				CERTIFICATE_VERIFY = new Type(15),
				CLIENT_KEY_EXCHANGE = new Type(16), FINISHED = new Type(20),
				CERTIFICATE_URL = new Type(21), CERTIFICATE_STATUS = new Type(
						22);
