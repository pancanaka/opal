...
public class SRPTrustManagerFactory extends TrustManagerFactorySpi {
	...
	protected void engineInit(ManagerFactoryParameters params)
		...
		if (params == null) {
			try {
				String srpPasswd = Util
						.getSecurityProperty("jessie.srp.password.file");
				if (srpPasswd == null) {
					current = new Manager(new PasswordFile());
					return;
				}
				String srpPasswd2 = Util
						.getSecurityProperty("jessie.srp.password.file2");
				if (srpPasswd2 == null)
					srpPasswd2 = srpPasswd + "2";
				String srpConfig = Util
						.getSecurityProperty("jessie.srp.config");
				if (srpConfig == null)
					srpConfig = srpPasswd + ".conf";
				current = new Manager(new PasswordFile(srpPasswd, srpPasswd2,
						srpConfig));
				return;
			} catch (IOException ioe) {
				throw new InvalidAlgorithmParameterException(
						"default initialization failed: " + ioe.toString());
			}
		}
		if (params instanceof SRPManagerParameters) {
			current = new Manager(
					((SRPManagerParameters) params).getPasswordFile());
			return;
		}
		throw new InvalidAlgorithmParameterException();
	}
	...
	private class Manager implements SRPTrustManager {
		...
		public KeyPair getKeyPair(String user) {
			try {
				if (file.contains(user)) {
					...
					BigInteger v, N, g;
					v = new BigInteger(1,
							gnu.crypto.util.Util.fromBase64(ent[0]));
					N = new BigInteger(1,
							gnu.crypto.util.Util.fromBase64(cnf[0]));
					g = new BigInteger(1,
							gnu.crypto.util.Util.fromBase64(cnf[1]));
		...
		public byte[] getSalt(String user) {
			try {
				if (file.contains(user)) {
					return gnu.crypto.util.Util.fromBase64(file.lookup(user,
							"SHA")[1]);
				}
			} catch (IOException ioe) {
			}
			...
			try {
				if (file.contains(user)) {
					return new BigInteger(1,
							gnu.crypto.util.Util.fromBase64(file.lookup(user,
									"SHA")[0]));
				}
			} catch (IOException ioe) {
			}
