...
public final class Session implements SSLSession {
	...
	private static final SSLPermission GET_SESSION_CONTEXT_PERMISSION = new SSLPermission(
	...
	public Session(long creationTime) {
		...
		values = new HashMap();
		if (("true").equalsIgnoreCase(Util
				.getSecurityProperty("jessie.with.jce")))
	...
	protected Object clone() {
		...
		result.localCerts = (localCerts != null ? (Certificate[]) localCerts
				.clone() : null);
		result.peerCerts = (peerCerts != null ? (Certificate[]) peerCerts
				.clone() : null);
	...
	public X509Certificate[] getPeerCertificateChain()
		...
		try {
			...
			for (int i = 0; i < peerCerts.length; i++) {
				peerCertChain[i] = X509Certificate.getInstance(peerCerts[i]
						.getEncoded());
			}
			return (X509Certificate[]) peerCertChain.clone();
	...
	static final class ID implements Comparable {
		 ...
		 * 
		 * @param id
		 *            The ID. The array is not cloned.
		 */
		ID(byte[] id) {
			if (id == null) {
