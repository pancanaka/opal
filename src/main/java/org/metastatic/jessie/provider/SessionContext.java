...
/**
 * A collection of SSL sessions. This implementation is a memory-only store;
 * subclasses may implement persistent storage.
 */
public class SessionContext implements SSLSessionContext {
	...
	public SessionContext() {
		...
		cacheSize = 0;
		try {
			timeout = Integer.parseInt(Util
					.getSecurityProperty("jessie.session.timeout"));
		} catch (Exception x) {
	 ...
	 * <li>Will do nothing if the cache limit has been reached (and is not
	 * zero).</li>
	 * <li>Will remove any invalid sessions in the cache before trying to insert
	 * the new one.</li>
	 * <li>Will remove any expired sessions before trying to insert the new one.
	 * </li>
	 * </ol>
	 * 
	 * @param sessionId
