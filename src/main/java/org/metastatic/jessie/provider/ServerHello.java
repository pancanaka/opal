...
class ServerHello implements Handshake.Body {
	...
	ServerHello(ProtocolVersion version, Random random, byte[] sessionId,
			CipherSuite suite, CompressionMethod comp) {
		this(version, random, sessionId, suite, comp, null);
	}

	ServerHello(ProtocolVersion version, Random random, byte[] sessionId,
			CipherSuite suite, CompressionMethod comp, List extensions) {
