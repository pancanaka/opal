...
final class CertificateRequest implements Handshake.Body {
	...
	static CertificateRequest read(InputStream in) throws IOException {
		...
		try {
			...
			Constructor c = x500class.getConstructor(new Class[] { new byte[0]
					.getClass() });
		...
		return new CertificateRequest(types,
				(Principal[]) authorities.toArray(new Principal[authorities
						.size()]));
	}
	...
	String[] getTypeStrings() {
		try {
			return (String[]) Util.transform(types, String.class, "toString",
					null);
		} catch (Exception x) {
			return null;
		}
	...
	static final class ClientType implements Enumerated {
		...
		static final ClientType RSA_SIGN = new ClientType(1),
				DSS_SIGN = new ClientType(2), RSA_FIXED_DH = new ClientType(3),
				DSS_FIXED_DH = new ClientType(4);
