	...
	public PrivateKey engineGeneratePrivate(KeySpec spec)
		...
		try {
			...
			if (algId.equals(ID_DSA)) {
				...
				return new GnuDSAPrivateKey((BigInteger) dsaPriv.read()
						.getValue(), dsaSpec.getP(), dsaSpec.getQ(),
						dsaSpec.getG());
			} else if (algId.equals(ID_RSA)) {
				...
				return new GnuRSAPrivateKey(new RSAPrivateCrtKeySpec(
						...
						(BigInteger) rsaParams.read().getValue(), // d mod (p -
																	// 1)
						(BigInteger) rsaParams.read().getValue(), // d mod (q -
																	// 1)
						(BigInteger) rsaParams.read().getValue())); // (inv q)
																	// mod p
			} else
				throw new InvalidKeySpecException("unknown algorithm: " + algId);
