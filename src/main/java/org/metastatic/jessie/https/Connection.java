 ...
 * gnu.java.net.protocol.http.HttpURLConnection from GNU Classpath, written by
 * Aaron M. Renn.
 */
class Connection extends HttpsURLConnection {
	...
	public synchronized void connect() throws IOException {
		...
		if (socket == null) {
			...
			int proxyPort = Integer.getInteger("https.proxyPort", 80)
					.intValue();
			if (proxyHost != null)
				socket = (SSLSocket) factory.createSocket(
						getProxySocket(host, port, proxyHost, proxyPort), host,
						port, true);
			else
	...
	private void sendRequest() throws IOException {
		...
		if (getRequestProperty("Content-length") == null && bufferedOut != null) {
			setRequestProperty("Content-length",
					String.valueOf(bufferedOut.size()));
		}
	...
	private static void putKeepAliveSocket(String host, int port,
			SSLSocket socket) {
		synchronized (keptAlive) {
			String key = host + ":" + port;
