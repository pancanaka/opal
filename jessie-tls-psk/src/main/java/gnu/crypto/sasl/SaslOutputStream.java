    ...
    public void write(byte[] b, int off, int len) throws IOException {
        ...
        while (len > 0) {
            ...
            if (DEBUG && debuglevel > 6)
                debug(TRACE, "Outgoing buffer (before security) (hex): " + Util.dumpString(b, off, chunckSize));
            if (DEBUG && debuglevel > 6)
                debug(TRACE, "Outgoing buffer (before security) (str): \"" + new String(b, off, chunckSize) + "\"");
