 ...
 * <p/>
 * These classes ({@link java.security.Signature} for example) can be thought of
 * as the "chrome, upholstery, and steering wheel", and the SPI (service
 * provider interface, e.g. {@link java.security.SignatureSpi}) classes can be
 * thought of as the "engine" -- providing the actual functionality of whatever
 * cryptographic algorithm the instance represents.
 *
...
final class Engine {
    ...
    /**
     * Get the implementation for <i>algorithm</i> for service <i>service</i>
     ...
     * @throws NoSuchAlgorithmException  If the implementation cannot be found or cannot be
     *                                   instantiated.
     * @throws InvocationTargetException If the SPI class's constructor throws an exception.
    ...
    static Object getInstance(String service, String algorithm,
                              Provider provider) throws InvocationTargetException,
     ...
     * @param algorithm The name of the algorithm to get.
     * @param provider  The provider to get the implementation from.
     * @param initArgs  The arguments to pass to the SPI class's constructor (cannot
     *                  be null).
     * @return The engine class for the specified algorithm; the object returned
     *         is typically a subclass of the SPI class for that service, but
     *         callers should check that this is so.
    ...
    static Object getInstance(String service, String algorithm,
        ...
        if (provider.getProperty(service + "." + algorithm) == null) {
            ...
            String alias = null;
            int count = 0;
            while ((alias = provider.getProperty(ALG_ALIAS + service + "."
                    + algorithm)) != null) {
        ...
        try {
            if (loader != null)
                clazz = loader.loadClass(provider.getProperty(service + "."
                        + algorithm));
            else
                clazz = Class.forName(provider.getProperty(service + "."
                        + algorithm));
     ...
     * @throws NoSuchMethodException If no constructor of the given class can take the specified
     *                               argument array.
     */
    private static Constructor getCompatibleConstructor(Class clazz,
                                                        Object[] initArgs) throws NoSuchMethodException {
