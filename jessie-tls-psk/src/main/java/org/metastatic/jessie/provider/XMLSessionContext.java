...
class XMLSessionContext extends SessionContext {
    ...
    XMLSessionContext() throws IOException, SAXException {
        file = new File(
                Util.getSecurityProperty("jessie.SessionContext.xml.file"));
        String password = Util
                .getSecurityProperty("jessie.SessionContext.xml.password");
        compress = new Boolean(
                Util.getSecurityProperty("jessie.SessionContext.xml.compress"))
                .booleanValue();
        if (password == null) {
            password = "";
        }
    ...
    private void encode() throws IOException {
        ...
        if (compress) {
            out = new PrintStream(new GZIPOutputStream(new FileOutputStream(
                    file)));
        } else {
    ...
    private class SAXHandler extends DefaultHandler {
        ...
        public void startElement(String u, String n, String qname,
            ...
            switch (state) {
                ...
                case SESSIONS:
                    if (qname.equals("session")) {
                        try {
                            current = new Session(Long.parseLong(attr
                                    .getValue("created")));
                            current.enabledSuites = new ArrayList(
                                    SSLSocket.supportedSuites);
                            current.enabledProtocols = new TreeSet(
                                    SSLSocket.supportedProtocols);
                            current.random = new SecureRandom();
                            current.context = context;
                            current.sessionId = new Session.ID(
                                    Base64.decode(attr.getValue("id")));
                            current.setLastAccessedTime(Long.parseLong(attr
                                    .getValue("timestamp")));
                        } catch (Exception ex) {
                        ...
                        current.cipherSuite = CipherSuite.forName(attr
                                .getValue("suite"));
                        state = SESSION;
                    } else
                ...
                case PEER_CERTS:
                    if (qname.equals("certificates")) {
                        try {
                            ...
                            current.peerCerts = (Certificate[]) fact
                ...
                case CERTS:
                    if (qname.equals("certificates")) {
                        try {
                            CertificateFactory fact = CertificateFactory
                                    .getInstance(certType);
                            current.localCerts = (Certificate[]) fact
                                    .generateCertificates(
                                            new ByteArrayInputStream(buf
                                                    .toString().getBytes()))
                                    .toArray(new Certificate[0]);
                        ...
                        try {
                            ...
                            if (encrypted.length != 68)
                                throw new IOException(
                                        "encrypted secret not 68 bytes long");
                            pbekdf.nextBytes(key, 0, key.length);
                            pbekdf.nextBytes(iv, 0, iv.length);
                            pbekdf.nextBytes(mackey, 0, mackey.length);
                            cipher.reset();
