...
class JCESecurityParameters implements SecurityParameters {
    ...
    public synchronized byte[] encrypt(byte[] fragment, int off, int len,
                                       ContentType type) throws OverflowException, SSLException {
        if (deflater != null) {
            byte[] buf = new byte[1024];
        ...
        if (outCipher != null) {
            try {
                ...
                // This is nonsensical. Don't even pretend that we can handle
                // this.
                throw new RuntimeException(
                        "bad padding thrown while encrypting");
            } catch (IllegalBlockSizeException ibse) {
                // Ditto.
                throw new RuntimeException(
                        "illegal block size thrown while encrypting");
            }
            off = 0;
            len = fragment.length;
        }
