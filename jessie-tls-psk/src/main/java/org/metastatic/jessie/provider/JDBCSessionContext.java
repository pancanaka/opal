...
/**
 * The SQL table this class stores sessions in, called <tt>SESSIONS</tt>, looks
 ...
 * </blockquote>
 * <p/>
 * Note that the master secret for sessions is not protected before being
 * inserted into the database; it is up to the system to protect the stored data
...
class JDBCSessionContext extends SessionContext {
        ...
        if (session == null) {
            try {
                ...
                if (rs.next()) {
                    ...
                    session.cipherSuite = CipherSuite.forName(rs
                            .getString("SUITE"));
                    ...
                    if (!wasNull) {
                        CertificateFactory cf = CertificateFactory
                                .getInstance(type);
                        session.peerCerts = (Certificate[]) cf
                                .generateCertificates(certs).toArray(
                                        new Certificate[0]);
                    ...
                    if (!wasNull) {
                        CertificateFactory cf = CertificateFactory
                                .getInstance(type);
                        session.localCerts = (Certificate[]) cf
                                .generateCertificates(certs).toArray(
                                        new Certificate[0]);
                    }
    ...
    synchronized void notifyAccess(Session session) {
        try {
            updateTimestamp.setTimestamp(1,
