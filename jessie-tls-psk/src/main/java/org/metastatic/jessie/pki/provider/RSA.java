...
public abstract class RSA extends SignatureSpi implements Cloneable {
     ...
     * rsadsi(113549) digestAlgorithm(2) }
     */
        ...
        if (ed.length < k) {
            ...
            if (ed.length != k + 1) {
                throw new SignatureException(
                        "modPow result is larger than the modulus");
            }
    ...
    protected boolean engineVerify(byte[] sig) throws SignatureException {
        ...
        if (sig.length != k)
            throw new SignatureException(
                    "signature is the wrong size (expecting " + k
                            + " bytes, got " + sig.length + ")");
        ...
        try {
            ...
            if (val.getTag() != DER.SEQUENCE)
                throw new SignatureException(
                        "failed to parse DigestAlgorithmIdentifier");
            boolean sequenceIsBer = val.getLength() == 0;
            val = der.read();
            if (val.getTag() != DER.OBJECT_IDENTIFIER)
                throw new SignatureException(
                        "failed to parse object identifier");
